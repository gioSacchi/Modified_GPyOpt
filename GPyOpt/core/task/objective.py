...
class SeparatedObjective(Objective):
    """
    Class to handle problems with separated objective function, i.e. and objective function made up by an unchanging function (composition).

    param func: objective function.
    param separated_function: inner function to the objective function (model).
    param batch_size: size of the batches (default, 1)
    param num_cores: number of cores to use in the process of evaluating the objective (default, 1).
    param objective_name: name of the objective function.
    param batch_type: Type of batch used. Only 'synchronous' evaluations are possible at the moment.
    ...
    .. Note:: the objective function should take 2 inputs, first 2-dimensional numpy arrays and second the corresponding values
    of the separated function, also as 2-dimensional numpy arrays. The outputs are the same dimensions. Each row should
    contain a location (in the case of the inputs) or a function evaluation (in the case of the outputs).
    """

    def __init__(self, func, separated_function, num_cores = 1, objective_name = 'no_name', batch_type = 'synchronous', space = None):
        self.func  = func
        self.separated_function = separated_function
        self.n_procs = num_cores
        self.num_evaluations = 0
        self.space = space
        self.objective_name = objective_name
    ...
    def evaluate(self, x):
        ...
        if self.n_procs == 1:
            sep_f_evals, f_evals, cost_evals = self._eval_func(x)
        else:
            try:
                f_evals, cost_evals = self._syncronous_batch_evaluation(x)
            except:
        ...
        return np.hstack([sep_f_evals, f_evals]), cost_evals
    ...
    def _eval_func(self, x):
        ...
        First separated function is evaluated, then the objective function.
        """
        cost_evals = []
        f_evals = np.empty(shape=[0, 1])
        sep_f_evals = np.empty(shape=[0, 1])

        for i in range(x.shape[0]):
            st_time    = time.time()
            separated_function_eval = self.separated_function(np.atleast_2d(x[i]))
            rlt = self.func(np.atleast_2d(x[i]), np.atleast_2d(separated_function_eval))
            sep_f_evals = np.vstack([sep_f_evals, separated_function_eval])
            f_evals     = np.vstack([f_evals,rlt])
            cost_evals += [time.time()-st_time]
        return sep_f_evals, f_evals, cost_evals
